...
- `robocopy /MIR`로 설치 폴더를 새 릴리즈 내용과 완전히 동일하게 맞춘다 (신규/변경 파일 복사 + 새 릴리즈에 없는 파일은 삭제). 처음엔 `/E`(하위 폴더 복사, 기존 파일 유지)였으나, exe 파일명이 `MapleWindow.App.exe` → `MapleWindow.exe`로 바뀌었던 v1.0.0→v1.0.1 업데이트에서 실제 버그로 드러났다: `/E`는 기존 파일을 지우지 않으므로 이름이 바뀐 옛 exe가 설치 폴더에 그대로 남고, 아래처럼 재실행 인자로 "현재 실행 중이던 exe 경로"를 그대로 넘기면 그 남아있는 구버전 exe가 다시 실행되어 — 이미 고쳤던 버그(예: 캐릭터 변경 후 스케줄러 대사 리셋 안 됨)가 신규 버전을 받은 뒤에도 재발했다. `/MIR`이 이런 사례를 원천적으로 막는다. 설치 폴더에는 앱 바이너리만 있고 사용자 데이터는 AppData/LocalAppData에 저장되므로(§3 UninstallService 참고) `/MIR`로 폴더를 완전히 새 릴리즈와 동일하게 맞춰도 안전하다.
- 재실행 대상 exe는 업데이트 시작 시점의 `Environment.ProcessPath`(구버전 실행 파일 경로)가 아니라, 방금 받은 릴리즈의 `copySource`에서 실제로 찾은 `*.exe` 파일명이다 — 이래야 exe 이름이 다시 바뀌는 미래의 릴리즈에서도 정확한 새 실행 파일을 재실행한다.
