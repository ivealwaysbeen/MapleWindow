...
public class OnceDailyStateStoreTests : IDisposable
{
    ...
    public void WasShownToday_NeverMarked_ReturnsFalse()
    {
        ...
        Assert.False(store.WasShownToday(OcidA, "몬스터 파크", Today));
    }
    ...
    public void MarkShown_ThenWasShownToday_ReturnsTrueForSameDate()
    {
        ...
        store.MarkShown(OcidA, "몬스터 파크", Today);

        Assert.True(store.WasShownToday(OcidA, "몬스터 파크", Today));
    }
    ...
    public void MarkShown_ThenWasShownToday_ReturnsFalseForDifferentDate()
    {
        ...
        Assert.False(store.WasShownToday(OcidA, "몬스터 파크", Today.AddDays(1)));
    }

    [Fact]
    public void MarkShown_PersistsAcrossInstances()
    {
        new OnceDailyStateStore(_tempPath).MarkShown(OcidA, "몬스터 파크", Today);

        var reloaded = new OnceDailyStateStore(_tempPath);

        Assert.True(reloaded.WasShownToday(OcidA, "몬스터 파크", Today));
    }

    [Fact]
    public void MarkShown_ForOneCharacter_DoesNotAffectAnotherCharacterWithSameContentName()
    {
        var store = new OnceDailyStateStore(_tempPath);

        store.MarkShown(OcidA, "몬스터 파크", Today);

        Assert.False(store.WasShownToday(OcidB, "몬스터 파크", Today));
    }
}
